...
class SalaryDeleteViewTests(EDMSTestCase):
    def setUp(self) -> None:
        self.salary = SalaryFactory.create()
        self.view_url = reverse_lazy("delete-salary", kwargs={"pk": self.salary.pk})
        self.redirect_login_url = f"{reverse_lazy('login')}?next={self.view_url}"
        self.success_url = reverse_lazy("list-salary")
        self.template_name = "employees/salaries/salary_delete.html"
