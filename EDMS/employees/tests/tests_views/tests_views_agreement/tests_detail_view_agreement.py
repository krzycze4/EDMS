...
class AgreementDetailViewTests(EDMSTestCase):
    def setUp(self) -> None:
        super().setUp()
        self.agreement = AgreementFactory.create()
        self.view_url = reverse_lazy("detail-agreement", kwargs={"pk": self.agreement.pk})
        self.not_logged_user_url = f"{reverse_lazy('login')}?next={self.view_url}"
        self.template_name = "employees/agreements/agreement_detail.html"
