...
class AgreementDeleteViewTests(EDMSTestCase):
    def setUp(self) -> None:
        super().setUp()
        self.agreement = AgreementFactory.create()
        self.view_url = reverse_lazy("delete-agreement", kwargs={"pk": self.agreement.pk})
        self.login_redirect_url = f"{reverse_lazy('login')}?next={self.view_url}"
        self.template_name = "employees/agreements/agreement_delete.html"
    ...
    def test_deny_delete_agreement_when_logged_user_group_accountants_execute_post_method(self):
        login = self.client.login(email=self.accountant.email, password=self.password)
        self.assertTrue(login)
        expected_value = 1
    ...
    def test_delete_agreement_when_logged_user_group_ceos_execute_post_method(self):
        login = self.client.login(email=self.ceo.email, password=self.password)
        self.assertTrue(login)
        expected_value = 1
        self.assertEqual(Agreement.objects.count(), expected_value)
    ...
    def test_deny_delete_agreement_when_logged_user_group_managers_execute_post_method(self):
        login = self.client.login(email=self.manager.email, password=self.password)
        self.assertTrue(login)
        expected_value = 1
        self.assertEqual(Agreement.objects.count(), expected_value)
    ...
    def test_delete_agreement_when_logged_user_group_hrs_execute_post_method(self):
        login = self.client.login(email=self.hr.email, password=self.password)
        self.assertTrue(login)
        expected_value = 1
        self.assertEqual(Agreement.objects.count(), expected_value)
