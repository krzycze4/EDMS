...
class TestCaseCustomUserManager(TestCase):
    def setUp(self):
        ...
        self.user = UserFactory.stub(password=self.password)

    def test_create_user(self):
        user = User.objects.create_user(email=self.user.email, password=self.password)
    ...
    def test_create_superuser(self):
        superuser = User.objects.create_superuser(email=self.user.email, password=self.password)
        self.assertEqual(superuser.email, self.user.email)
    ...
    def test_create_superuser_failure_is_staff_false(self):
        with self.assertRaises(ValueError) as context:
            User.objects.create_superuser(email=self.user.email, password=self.password, is_staff=False)
    ...
    def test_create_superuser_failure_is_superuser_false(self):
        with self.assertRaises(ValueError) as context:
            User.objects.create_superuser(email=self.user.email, password=self.password, is_superuser=False)
        self.assertTrue(str(context.exception), "Superuser must have is_superuser=True.")

    def test_get_by_natural_key(self):
        self.user = User.objects.create_user(email=self.user.email, password=self.password)
        user_get_by_natural_key = User.objects.get_by_natural_key(email=self.user.email)
...
class TestCaseUser(TestCase):
    ...
    def test_str(self):
        self.assertEqual(f"{self.user.first_name} {self.user.last_name}", str(self.user))
