...
def render_plot(orders: List[Order], text: str, is_company=False) -> str:
    plot_data = set_plot_data(orders=orders, is_company=is_company)
    x_data = set_x_values(plot_data=plot_data)
...
def set_plot_data(
    orders: List[Order], is_company: bool
    ...
    if len(orders) > 0:
        ...
        while start_month <= end_month and start_year <= end_year:
            ...
            for order in orders:
                if (
                    order.end_date.month <= start_month
                    and order.end_date.year <= start_year
                ):
                    balance: Decimal = count_order_balance(
                        order=order, is_company=is_company
                    )
...
def count_order_balance(order: Order, is_company: bool) -> Decimal:
    income_invoices = order.income_invoice.all()
    income_sum_net_price = get_sum_invoice_net_price(invoices=income_invoices)

    cost_invoices = order.cost_invoice.all()
    cost_sum_net_price = get_sum_invoice_net_price(invoices=cost_invoices)

    sum_payment_fee = get_payment_fees(is_company=is_company, order=order)
...
def get_payment_fees(is_company, order):
    ...
    if is_company:
        payments = Payment.objects.filter(
            date__month=order.end_date.month, date__year=order.end_date.year
        )
    else:
        payments = Payment.objects.filter(
